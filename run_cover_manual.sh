...
#####################################################################
# run_cover_manual.sh $HOME/User/folder_repository programs.txt
# 
# Run the adequate test set for manual programs them you need to run
# manual_check-equiv.sh to check all live mutants as equivalent.
#####################################################################


if [ "$#" -ne 2 ]
then
  echo "Usage: run_cover_manual.sh <projects root directory> <file with directory names to be tested>"
  exit 1
else
   EXPER_HOME=$1
   FILES=$2
