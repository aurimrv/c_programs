...
{ 
char v[20], *s; 
int M = 0, digit; ponta_de_prova (8950, 1);
    
    while  ( ponta_de_prova (8950, 2), (N != 0) )
        { 
            ponta_de_prova (8950, 3);
            M = (10 * M) + (N % 10); 
            N = N / 10; 
        } 
    ponta_de_prova (8950, 4);
    printf("%d\n", M); 
    s = v; 
    
    while  ( ponta_de_prova (8950, 5), (M != 0) )
        { 
            ponta_de_prova (8950, 6);
            digit = M % 10; 
            s[0] = '0' + digit; 
            s++; 
            M = M / 10; 
        } 
    ponta_de_prova (8950, 7);
    s[0] = '\0'; 
    printf("%s\n", v); 
} 
void main(int argc, char *argv[]) 
{ 
int N = atoi(argv[1]); ponta_de_prova (9303, 1);
    digit_reverser(N); 
} 
